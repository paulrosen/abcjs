    ...
    this.cleanUp = function(defWidth, defLength, barsperstaff, staffnonote) {
        ...
        if (anyDeleted) {
            ...
            window.ABCJS.parse.each(this.lines, function(line) {
                if (line.staffs)
                    line.staffs = window.ABCJS.parse.compact(line.staffs);
            });
        }
            ...
            var addEndSlur = function(obj, num, chordPos) {
                if (currSlur[chordPos] === undefined) {
                    ...
                    if (currSlur[chordPos] === undefined) {
                        ...
                        window.ABCJS.parse.each(obj.endSlur, function(x) {
                            if (offNum === x)
                                --offNum;
                        });
...
//					lyr.syllable += '<' + slurNum;	// TODO-PER: debugging
            ...
            var addStartSlur = function(obj, num, chordPos, usedNums) {
                ...
                for (var i = 0; i < num; i++) {
                    if (usedNums) {
                        window.ABCJS.parse.each(usedNums, function(x) {
                            if (nextNum === x)
                                ++nextNum;
                        });
                        window.ABCJS.parse.each(usedNums, function(x) {
                    ...
                    window.ABCJS.parse.each(currSlur[chordPos], function(x) {
                        if (nextNum === x)
                            ++nextNum;
                    });
                    window.ABCJS.parse.each(currSlur[chordPos], function(x) {
...
//				lyr.syllable = '';	// TODO-PER: debugging
        ...
        if (this.hasTablature) {
            ...
            if (this.lines[0].staffs[staffTab].voices[0].length === 0) {
                ...
                (staffTab === 0) && this.warnings.push("+Warn: Accordion Tablature should be the first staff!");
...
//			}
    ...
    this.appendStartingElement = function(type, startChar, endChar, hashParams2)
    {
        ...
        if (this.lines[this.lineNum].staffs.length <= this.staffNum) {
            ...
            this.lines[this.lineNum].staffs[this.staffNum].clef = window.ABCJS.parse.clone(this.lines[this.lineNum].staffs[0].clef);
            this.lines[this.lineNum].staffs[this.staffNum].key = window.ABCJS.parse.clone(this.lines[this.lineNum].staffs[0].key);
            this.lines[this.lineNum].staffs[this.staffNum].meter = window.ABCJS.parse.clone(this.lines[this.lineNum].staffs[0].meter);
            this.lines[this.lineNum].staffs[this.staffNum].workingClef = window.ABCJS.parse.clone(this.lines[this.lineNum].staffs[0].workingClef);
