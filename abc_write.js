...
var AbcGlyphs = Class.create({
	initialize: function(paper) {
		...
		this.getSymbolWidth = function (symbol, path) {
		  return glyphs[symbol].w;
		};

		this.getSymbolHeight = function (symbol, path) {
...
function ABCPrinter(paper) {
  ...
  this.font = this.paper.getFont("Maestro", 500);
  this.glyphs = new AbcGlyphs(paper);
}

ABCPrinter.pathcache = {};
...
ABCPrinter.prototype.printBeam = function() {
  ...
  if (this.nextElemType() === 'note') {
    ...
    for (;;) {
      ...
      this.pos++;
      this.x += graphelem.getSpace(this.space);
    }
...
//  var start_x = this.x;
//
//  if (elem.type==="bar_left_repeat" || elem.type==="bar_double_repeat") {
...
//  }
//
//  if (elem.type==="bar_thin" || elem.type=== "bar_thin_thick" || elem.type=== "bar_thin_thin") {
//    elemset.push(this.glyphs.printSymbol(this.x, this.calcY(3), "\\")); // 3 is hardcoded
//    this.x+=this.glyphs.getSymbolWidth("\\",symb);
//  }
//
//  if (elem.type=== "bar_thin_thin") {
//    elemset.push(this.glyphs.printSymbol(this.x, this.calcY(3), "\\")); // 3 is hardcoded
//    this.x+=this.glyphs.getSymbolWidth("\\",symb);
//  }
//
//  if (elem.type=== "bar_thin_thick") {
//    elemset.push(this.glyphs.printSymbol(this.x, this.calcY(3), "\\")); // 3 is hardcoded
//    this.x+=this.glyphs.getSymbolWidth("\\",symb);
//  }
...
//    this.glyphs.getSymbolWidth("\\",symb);
...
//  }
//
//  if (this.partstartx && elem.type==="thin-thick") {
//    this.paper.path(sprintf("M %f %f L %f %f L %f %f",
//			      this.x, this.y+10, this.x, this.y, this.partstartx, this.y)).attr({stroke:"#000000"});
//    this.partstartx = null;
//  }
//
//
//  if (elem.type==="thin-thick" || elem.type==="repeat-left-right" || elem.type==="bar_left_repeat") {
//    this.x+=3; //3 hardcoded;
//    symb = this.glyphs.printSymbol(this.x, this.calcY(3), "\\"); // 3 is hardcoded
//    symbscale = 1;
//    elemset.push(symb);
//  }
//
//  if (elem.type==="bar_left_repeat" || elem.type==="repeat-left-right") {
...
//    this.paper.text(this.x+5,this.y+7,elem.barline.number);
//  }

  //return new ABCGraphElem(elemset, null, this.x - start_x, 0.5);
//  return new ABCGraphElem(elemset, null, this.glyphs.getSymbolWidth("\\",symb)*symbscale, 0.5);
};

ABCPrinter.prototype.printStave = function (width) {
  var staff = this.glyphs.printSymbol(0, this.calcY(3), "="); // 3 is hardcoded
  width = width/(this.glyphs.getSymbolWidth("=",staff));
  staff.scale(width,1,0);
};

ABCPrinter.prototype.printKeySignature = function(elem) {
  var clef = this.glyphs.printSymbol(this.x, this.calcY(5), "&"); //5 is hardcoded
  ...
  for (var i=0; i<number; i++) {
    var path = this.glyphs.printSymbol(this.x, this.calcY(accidentals[i]+1), symbol); //1 is hardcoded
...
ABCPrinter.prototype.printTimeSignature= function(elem) {
  ...
  if (elem.type === "specified") {
    ...
    graphelem.push(this.glyphs.printSymbol(this.x, this.calcY(9), elem.num)); //7 is hardcoded
    graphelem.push(this.glyphs.printSymbol(this.x, this.calcY(5), elem.den)); //3 is hardcoded
  } else if (elem.type === "common_time") {
    graphelem = this.glyphs.printSymbol(this.x, this.calcY(7), "c"); //5 is hardcoded
