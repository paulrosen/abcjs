...
var TuneBuilder = function (tune) {
      ...
      if (line.staff) {
        for (var j = 0; j < line.staff.length; j++) {
          ...
          for (var k = 0; k < staff.voices.length; k++) {
              ...
              if (event.el_type === "overlay" && !inOverlay) {
                ...
                for (var ii = 0; ii < i; ii++) {
                  if (
                    ...
                    tune.lines[ii].staff &&
                    staff.voices.length >= tune.lines[ii].staff[0].voices.length
                  ) {
            ...
            if (
              overlayVoice[k].hasOverlay &&
              overlayVoice[k].snip.length === 0
            ) {
        ...
        for (var j = 0; j < line.staff.length; j++) {
          ...
          if (staff.title) {
            ...
            for (var k = 0; k < staff.title.length; k++) {
              if (staff.title[k]) {
                staff.title[k] = firstMusicLine
                  ? staff.title[k].name
                  : staff.title[k].subname;
        ...
        if (el.el_type === "note") {
          ...
          if (el.pitches) {
            ...
            if (
              ...
              el.pitches[0].endSlur[0] === 100 &&
              el.pitches[0].startSlur
            ) {
              ...
              if (el.pitches[0].endSlur.length === 1)
                ...
                el.pitches[0].endSlur[el.pitches[0].endSlur.length - 1] === 100
              )
                    ...
                    if (n < voice.length - 1) {
                      ...
                      if (!nextLine) {
                        ...
                        for (var ss = 0; ss < nextLine.staff.length; ss++) {
                          for (
                            var vv = 0;
                            vv < nextLine.staff[ss].voices.length;
                            vv++
                          )
                      ...
                      var section =
                        lines[i].staff[s].voices[v].slice(startElement);
                      lines[i].staff[s].voices[v] = lines[i].staff[s].voices[
                        v
                      ].slice(0, startElement);
                      nextLine.staff[s].voices[v] = permanentItems.concat(
                        section.concat(nextLine.staff[s].voices[v])
                      );
                      return true;
                    }
                  }
    ...
    for (tune.lineNum = 0; tune.lineNum < tune.lines.length; tune.lineNum++) {
      ...
      if (staff) {
        for (tune.staffNum = 0; tune.staffNum < staff.length; tune.staffNum++) {
          ...
          for (
            tune.voiceNum = 0;
            tune.voiceNum < staff[tune.staffNum].voices.length;
            tune.voiceNum++
          ) {
  ...
  this.appendElement = function (type, startChar, endChar, hashParams) {
    ...
    var endBeamLast = function () {
      if (
        This.potentialStartBeam !== undefined &&
        This.potentialEndBeam !== undefined
      ) {
    ...
    if (type === "note") {
      ...
      if (dur >= 0.25) {
        ...
        hashParams.force_end_beam_last &&
        This.potentialStartBeam !== undefined
      ) {
        endBeamLast();
    ...
    if (tune.lines[tune.lineNum] && tune.lines[tune.lineNum].staff) {
      ...
      for (var i = 0; i < voice.length; i++) {
        if (voice[i].el_type === "note" || voice[i].el_type === "bar") {
          ...
          if (impliedNaturals)
            hashParams.accidentals = impliedNaturals.concat(
              hashParams.accidentals
            );
  ...
  this.containsNotesStrict = function (voice) {
    for (var i = 0; i < voice.length; i++) {
      if (
        ...
        (voice[i].rest === undefined || voice[i].chord !== undefined)
      )
        return true;
    }
    return false;
  ...
  this.startNewLine = function (params) {
    ...
    var createStaff = function (params) {
      ...
      if (params.stafflines !== undefined) {
        This.lines[This.lineNum].staff[This.staffNum].clef.stafflines =
          params.stafflines;
        This.lines[This.lineNum].staff[This.staffNum].workingClef.stafflines =
          params.stafflines;
      }
      if (params.staffscale) {
        This.lines[This.lineNum].staff[This.staffNum].staffscale =
          params.staffscale;
      }
      if (params.annotationfont)
        self.setLineFont("annotationfont", params.annotationfont);
      if (params.gchordfont) self.setLineFont("gchordfont", params.gchordfont);
      if (params.tripletfont)
        self.setLineFont("tripletfont", params.tripletfont);
      if (params.vocalfont) self.setLineFont("vocalfont", params.vocalfont);
      if (params.bracket)
        This.lines[This.lineNum].staff[This.staffNum].bracket = params.bracket;
      if (params.brace)
        This.lines[This.lineNum].staff[This.staffNum].brace = params.brace;
      if (params.connectBarLines)
        This.lines[This.lineNum].staff[This.staffNum].connectBarLines =
          params.connectBarLines;
      if (params.barNumber)
        This.lines[This.lineNum].staff[This.staffNum].barNumber =
          params.barNumber;
      createVoice(params);
    ...
    } else if (tune.lines[tune.lineNum].staff[tune.staffNum] === undefined)
      createStaff(params);
    else if (
      tune.lines[tune.lineNum].staff[tune.staffNum].voices[tune.voiceNum] ===
      undefined
    ...
    if (currentVoice && currentVoice.length > 0) {
      ...
      if (lastElement.el_type === "bar") {
        if (lastElement.barNumber !== undefined)
          // the measure number might not be written for tune bar, don't override that.
    ...
    for (var i = 0; i < tune.lines.length; i++) {
      if (tune.lines[i].staff) {
        if (
          ...
          tune.lines[i].staff[staffNum].voices[voiceNum] === undefined ||
