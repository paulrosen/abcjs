...
var Tokenizer = function (lines, multilineVars) {
  ...
  this.getMode = function (str) {
    ...
    if (
      (firstThree.length > 1 && firstThree.charAt(1) === " ") ||
      firstThree.charAt(1) === "^" ||
      firstThree.charAt(1) === "_" ||
      firstThree.charAt(1) === "="
    )
  ...
  this.getClef = function (str, bExplicitOnly) {
    ...
    if (parseCommon.startsWith(strClef, "treble")) name = "treble";
      ...
      name = "treble";
    else return { len: i + 5, warn: "Unknown clef specified: " + strOrig };
      ...
      case "]":
        ...
        switch (line.charAt(i)) {
          ...
          case "[":
            ++i;
            if (
              (line.charAt(i) >= "1" && line.charAt(i) <= "9") ||
              line.charAt(i) === '"'
            )
  ...
  this.getKeyAccidentals2 = function (tokens) {
    ...
    while (tokens.length > 0) {
      ...
      if (tokens[0].token === "^") {
        ...
        tokens.shift();
        if (tokens.length === 0)
          return { accs: accs, warn: "Expected note name after " + acc };
        switch (tokens[0].token) {
          case "^":
            acc = "dblsharp";
            tokens.shift();
      ...
      switch (tokens[0].token.charAt(0)) {
        ...
        default:
          return {
            accs: accs,
            warn:
              "Expected note name after " + acc + " Found: " + tokens[0].token
          };
      }
    }
    return { accs: accs };
  ...
  this.tokenize = function (line, start, end, alphaUntilWhiteSpace) {
    ...
    while (start < end) {
      if (line.charAt(start) === '"') {
        ...
        isNumber(line.charAt(start)) ||
        (line.charAt(start) === "-" && isNumber(line.charAt(i + 1)))
      ) {
        i = start + 1;
        var intt = null;
        var floatt = null;
  ...
  this.getVoiceToken = function (line, start, end) {
    ...
    var i = start;
    while (
      (i < end && this.isWhiteSpace(line.charAt(i))) ||
      line.charAt(i) === "="
    )
      i++;
    ...
    if (line.charAt(i) === '"') {
      ...
      while (
        ii < end &&
        !this.isWhiteSpace(line.charAt(ii)) &&
        line.charAt(ii) !== "="
      )
        ii++;
    ...
    if (line.charAt(index) === "/") {
      ...
      if (line.charAt(index) === "/") {
        ...
        if (ret2.num === 0 && iSave === index)
          // If we didn't use any characters, it is an implied 2
          ret2.num = 2;
