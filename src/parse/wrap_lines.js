    ...
    if (lines[action.ogLine].staff) {
      ...
      if (!outputLines[action.line].staff[action.staff]) {
        ...
        if (barNumbers !== undefined && action.staff === 0 && action.line > 0) {
          outputLines[action.line].staff[action.staff].barNumber =
            currentBarNumber;
        }
        ...
        for (var k = 0; k < keys.length; k++) {
          ...
          if (!skip)
            outputLines[action.line].staff[action.staff][keys[k]] =
              inputStaff[keys[k]];
        }
        if (lastKeySig[action.staff])
          outputLines[action.line].staff[action.staff].key =
      ...
      outputLines[action.line].staff[action.staff].voices[action.voice] = lines[
        action.ogLine
      ].staff[action.staff].voices[action.voice].slice(
        action.start,
        action.end + 1
      );
...
function oneTry(
  ...
  for (var i = startIndex; i < measureWidths.length; i++) {
    ...
    var varianceIsClose =
      Math.abs(thisVariance - lastVariance) < idealWidths[0] / 10; // see if the difference is less than 10%, if so, run the test both ways.
    if (varianceIsClose) {
...
function getRevisedTuneParams(lineBreaks, staffWidth, params) {
  ...
  for (var key in params) {
    if (
      params.prototype.hasOwnProperty.call(key) &&
      key !== "wrap" &&
      key !== "staffwidth"
    ) {
...
function calcLineWraps(tune, widths, params) {
  ...
  var minSpacing = params.wrap.minSpacing
    ? Math.max(parseFloat(params.wrap.minSpacing), 1)
    : 1;
  var minSpacingLimit = params.wrap.minSpacingLimit
    ? Math.max(parseFloat(params.wrap.minSpacingLimit), 1)
    : minSpacing - 0.1;
  var maxSpacing = params.wrap.maxSpacing
    ? Math.max(parseFloat(params.wrap.maxSpacing), 1)
    : undefined;
  ...
  var preferredMeasuresPerLine = params.wrap.preferredMeasuresPerLine
    ? Math.max(parseInt(params.wrap.preferredMeasuresPerLine, 10), 0)
    : undefined;
