...
MusicParser.prototype.parseMusic = function (line: any) {
  ...
  else multilineVars.start_new_line = false;
  ...
  while (i < line.length) {
    ...
    var retInlineHeader = header.letter_to_inline_header(
      line,
      i,
      delayStartNewLine
    );
    if (retInlineHeader[0] > 0) {
      ...
      if (
        ...
        (delayStartNewLine && !this.lineContinuation)
      ) {
        this.startNewLine();
      ...
      if (ret[0] > 0) {
        ...
        if (bar.type.length === 0) warn("Unknown bar type", line, i);
          ...
          if (bar.type !== "bar_invisible" && multilineVars.measureNotEmpty) {
            var isFirstVoice =
              multilineVars.currentVoice === undefined ||
              (multilineVars.currentVoice.staffNum === 0 &&
        ...
        if (line.charAt(i) === "[") {
          ...
          if (core !== null) {
            ...
            if (
              tripletNotesLeft > 0 &&
              !(core.rest && core.rest.type === "spacer")
            ) {
              tripletNotesLeft--;
              if (tripletNotesLeft === 0) {
                // @ts-expect-error TS(2339): Property 'endTriplet' does not exist on type '{}'.
            ...
            var durations = [
              0.5, 0.75, 0.875, 0.9375, 0.96875, 0.984375, 0.25, 0.375, 0.4375,
              0.46875, 0.484375, 0.4921875, 0.125, 0.1875, 0.21875, 0.234375,
              0.2421875, 0.24609375, 0.0625, 0.09375, 0.109375, 0.1171875,
              0.12109375, 0.123046875, 0.03125, 0.046875, 0.0546875, 0.05859375,
              0.060546875, 0.0615234375, 0.015625, 0.0234375, 0.02734375,
...
var letter_to_accent = function (line: any, i: any) {
  ...
  switch (line.charAt(i)) {
    ...
    case "+":
      ...
      if (
        ret[1].length > 1 &&
        (ret[1].charAt(0) === "^" || ret[1].charAt(0) === "_")
      )
...
var letter_to_bar = function (line: any, curr_pos: any) {
  ...
  if (
    line.charAt(curr_pos + ret.len) === '"' &&
    line.charAt(curr_pos + ret.len - 1) === "["
  ) {
    var ending = tokenizer.getBrackettedSubstring(line, curr_pos + ret.len, 5);
    return [ret.len + ending[0], ret.token, ending[1]];
  }
  var retRep = tokenizer.getTokenOf(
    line.substring(curr_pos + ret.len),
    "1234567890-,"
  );
...
MusicParser.prototype.startNewLine = function () {
  ...
  if (multilineVars.tempoForNextLine.length === 4)
    tuneBuilder.appendElement(
      multilineVars.tempoForNextLine[0],
      multilineVars.tempoForNextLine[1],
      multilineVars.tempoForNextLine[2],
      multilineVars.tempoForNextLine[3]
    );
    ...
    switch (line.charAt(index)) {
      ...
      case "g":
        if (
          ...
          if (canHaveBrokenRhythm && multilineVars.next_note_duration !== 0) {
            el.duration =
              multilineVars.default_length * multilineVars.next_note_duration;
            multilineVars.next_note_duration = 0;
          ...
          if (
            (multilineVars.clef && multilineVars.clef.type === "perc") ||
            (multilineVars.currentVoice &&
              multilineVars.currentVoice.clef === "perc")
          ) {
      ...
      case "/":
        if (state === "octave" || state === "duration") {
          ...
          while (
            fraction.index < line.length &&
            (tokenizer.isWhiteSpace(line.charAt(fraction.index)) ||
              line.charAt(fraction.index) === "-")
          ) {
            if (line.charAt(fraction.index) === "-") el.startTie = {};
            else el = addEndBeam(el);
            fraction.index++;
          }
      ...
      case "\t":
        if (isComplete(state)) {
          ...
          do {
            ...
            (index < line.length &&
              (tokenizer.isWhiteSpace(line.charAt(index)) ||
                line.charAt(index) === "-")) ||
            (line.charAt(index) === "." && line.charAt(index + 1) === "-")
          );
          el.endChar = index;
          if (
            !durationSetByPreviousNote &&
            canHaveBrokenRhythm &&
