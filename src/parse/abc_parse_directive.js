        ...
        case "face":
          if (
            ...
            if (currToken.type === "number") {
              ...
              warn(
                "Unknown parameter " + currToken.token + " in font definition.",
                str,
                position
              );
          }
  ...
  var parseMidiCommand = function (midi, tune, restOfString) {
    ...
    if (midiCmdParam0.indexOf(midi_cmd) >= 0) {
      ...
      if (midi.length !== 2)
        ...
        warn(
          "Expected one string and one integer parameters in MIDI " + midi_cmd,
          restOfString,
          0
        );
      else {
      ...
      if (midi.length !== 1 || midi.length !== 4)
        ...
        if (midi[3].type !== "number")
          warn(
            "Expected integer parameter for octave in MIDI " + midi_cmd,
            restOfString,
            0
          );
      } else {
  ...
  parseDirective.addDirective = function (str) {
    ...
    switch (cmd) {
      ...
      case "setbarnb":
        ...
        multilineVars.currBarNumber = tuneBuilder.setBarNumberImmediate(
          tokens[0].intt
        );
        break;
      case "begintext":
        ...
        var addContinueBar = function () {
          ...
          if (lastVoice) {
            ...
            if (lastVoice.staffNum > 0) {
              if (
                multilineVars.staves[lastVoice.staffNum - 1].connectBarLines ===
                  "start" ||
                multilineVars.staves[lastVoice.staffNum - 1].connectBarLines ===
                  "continue"
              )
      ...
      case "footer":
        ...
        if (
          footerStr.charAt(0) === '"' &&
          footerStr.charAt(footerStr.length - 1) === '"'
        )
        ...
        if (footerArr.length > 3)
          warn(
            "Too many tabs in " + cmd + ": " + footerArr.length + " found.",
            restOfString,
            0
          );
