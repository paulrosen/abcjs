...
var ParseHeader = function(
  ...
  this.setTitle = function (title: any) {
    if (multilineVars.hasMainTitle)
      ...
      var titleStr = tokenizer.translateString(
        tokenizer.theReverser(tokenizer.stripComment(title))
      );
  ...
  this.calcTempo = function (relTempo: any) {
    ...
    if (multilineVars.meter && multilineVars.meter.type === "specified") {
      ...
      multilineVars.origMeter.type === "specified"
    ) {
      ...
      switch (line.substring(i, i + 3)) {
        ...
        case "[P:":
          if (startLine || tune.lines.length <= tune.lineNum)
            ...
            tuneBuilder.appendElement("part", startChar, endChar, {
              title: line.substring(i + 3, e)
            });
          return [e - i + 1 + ws];
        case "[L:":
    ...
    if (field !== undefined) {
      ...
      switch (line.charAt(0)) {
        case "H":
          tuneBuilder.addMetaText(
            "history",
            tokenizer.translateString(
              tokenizer.stripComment(line.substring(2))
            ),
            {
              startChar: multilineVars.iChar,
              endChar: multilineVars.iChar + line.length
          ...
          while (line && line.charAt(1) !== ":") {
            ...
            tuneBuilder.addMetaText(
              "history",
              tokenizer.translateString(tokenizer.stripComment(line)),
              {
                startChar: multilineVars.iChar,
                endChar: multilineVars.iChar + line.length
              }
        ...
        case "L":
          ...
          break;
        case "M":
          multilineVars.origMeter = multilineVars.meter = this.setMeter(
            line.substring(2)
          );
          break;
        case "P":
          ...
          if (multilineVars.is_in_header)
            ...
            multilineVars.partForNextLine = {
              title: tokenizer.translateString(
                tokenizer.stripComment(line.substring(2))
              ),
              startChar: startChar,
              endChar: endChar
