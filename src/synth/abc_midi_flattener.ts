...
(function () {
      ...
      for (var j = 0; j < voice.length; j++) {
        ...
        switch (element.el_type) {
          ...
          default: // jshint ignore:line
            ...
            console.log(
              "MIDI creation. Unknown el_type: " + element.el_type + "\n"
            );
            break;
        }
      }
  ...
  function adjustPitch(note: any) {
    ...
    if (note.accidental) {
      switch (
        note.accidental // change that pitch (not other octaves) for the rest of the bar
      ) {
        case "sharp":
  ...
  function resolveChords(startTime: any, endTime: any) {
    ...
    if (currentChords[0].beat !== 0 && lastChord) {
      ...
      if (chordLastBar) currentChords.unshift({ beat: 0, chord: chordLastBar });
    }
