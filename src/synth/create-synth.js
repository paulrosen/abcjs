...
function CreateSynth() {
  ...
  self.init = function (options) {
    ...
    self.soundFontUrl = params.soundFontUrl
      ? params.soundFontUrl
      : defaultSoundFontUrl;
    ...
    if (
      ...
      params.soundFontVolumeMultiplier === 0
    )
      self.soundFontVolumeMultiplier = params.soundFontVolumeMultiplier;
    else if (
      self.soundFontUrl === defaultSoundFontUrl ||
      self.soundFontUrl === alternateSoundFontUrl
    )
    ...
    var p =
      params.fadeLength !== undefined ? parseInt(params.fadeLength, 10) : NaN;
    ...
    if (options.visualObj) {
      ...
      if (meter.den)
        self.meterSize =
          options.visualObj.getMeterFraction().num /
          options.visualObj.getMeterFraction().den;
    ...
    if (self.debugCallback)
      self.debugCallback(
        "note gathering time = " +
          Math.floor((activeAudioContext().currentTime - startTime) * 1000) +
          "ms"
      );
  ...
  self._loadBatch = function (batch, soundFontUrl, startTime, delay) {
    ...
    return Promise.all(promises)
      .then(function (response) {
        ...
        } else return Promise.resolve({ loaded: loaded, cached: cached, error: error });
      })
      .catch(function (error) {});
  };
  ...
  self.prime = function () {
    ...
    return new Promise(function (resolve) {
      ...
      var totalSamples = Math.floor(
        activeAudioContext().sampleRate * self.duration
      );
      ...
      var allPromises = [];
      var audioBuffer = activeAudioContext().createBuffer(
        2,
        totalSamples,
  ...
  self.seek = function (position, units) {
    ...
    switch (units) {
      ...
      case "beats":
        offset =
          (position * self.millisecondsPerMeasure) /
