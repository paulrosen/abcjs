...
function SynthController() {
  ...
  self.load = function (selector, cursorControl, visualOptions) {
    ...
    self.control = new CreateSynthControl(selector, {
      ...
      progressHandler: visualOptions.displayProgress
        ? self.randomAccess
        : undefined,
    ...
    return activeAudioContext()
      ...
      .then(function () {
        ...
        self.timer = new TimingCallbacks(self.visualObj, {
          ...
          extraMeasuresAtBeginning: self.cursorControl
            ? self.cursorControl.extraMeasuresAtBeginning
            : undefined,
          lineEndAnticipation: self.cursorControl
            ? self.cursorControl.lineEndAnticipation
            : 0,
          beatSubdivisions: subdivisions
        });
        if (
          self.cursorControl &&
          self.cursorControl.onReady &&
          typeof self.cursorControl.onReady === "function"
        )
      ...
      .then(function () {
        ...
        if (self.isStarted) {
          if (
            ...
            typeof self.cursorControl.onStart === "function"
          )
            self.cursorControl.onStart();
    ...
    if (self.isLooping) {
      ...
      if (self.isStarted) {
        ...
        if (
          ...
          typeof self.cursorControl.onFinished === "function"
        )
          self.cursorControl.onFinished();
        self.setProgress(0, 1);
      }
  ...
  self.beatCallback = function (beatNumber, totalBeats, totalTime, position) {
    ...
    if (
      self.cursorControl &&
      self.cursorControl.onBeat &&
      typeof self.cursorControl.onBeat === "function"
    )
  ...
  self.eventCallback = function (event) {
    if (event) {
      if (
        self.cursorControl &&
        self.cursorControl.onEvent &&
        typeof self.cursorControl.onEvent === "function"
      )
  ...
  self.lineEndCallback = function (lineEvent, leftEvent) {
    if (
      ...
      typeof self.cursorControl.onLineEnd === "function"
    )
