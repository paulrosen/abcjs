...
function lintOne(history, abcString) {
  ...
  if (
    history.absEl &&
    history.absEl.abcelem &&
    history.absEl.abcelem.startChar >= 0
  )
    originalText =
      "`" +
      abcString.substring(
        history.absEl.abcelem.startChar,
        history.absEl.abcelem.endChar
      ) +
      "`";
  originalText += " selectable";
  if (history.isDraggable) originalText += " draggable";
  else originalText += " not-draggable";
...
function listClasses(el) {
  ...
  if (klasses.length === 0) return "[none]";
  return "." + klasses.join(".");
}
    ...
    switch (keys[i]) {
      ...
      case "specialY":
        ...
        for (var y = 0; y < yKeys.length; y++) {
          if (item[yKeys[y]] !== 0) yVals.push(yKeys[y] + "=" + item[yKeys[y]]);
        }
        if (yVals.length > 0) output.push(keys[i] + ": " + yVals.join(", "));
        break;
      case "beam":
        if (item.length > 0) console.log(keys[i], item);
        break;
      case "heads":
      case "extra":
      case "children":
      ...
      case "isClef":
        ...
        if (item) output.push(keys[i] + ": " + item);
        break;
      default:
        if (item !== undefined) output.push(keys[i] + ": " + item);
    }
  }
  if (absEl.elemset) {
    item = absEl.elemset;
...
function listRelativeElement(relativeElement, indent) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    switch (keys[i]) {
      ...
      case "tempo":
        var tempoDetails = [];
        if (item.preString) tempoDetails.push("preString: " + item.preString);
        if (item.duration && item.duration.length)
          tempoDetails.push("duration: " + item.duration.join(","));
        if (item.bpm) tempoDetails.push("bpm: " + item.bpm);
        if (item.postString)
...
function listAbcElement(abcelem, indent) {
  ...
  if (ignored.length > 0) output.push("ignored: " + ignored.join(" "));
  return "\n" + tab + output.join("\n" + tab);
}
