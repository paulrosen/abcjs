  ...
  _validate: function (
          ...
          if (enumField === undefined)
            ...
            for (var tt = 0; tt < schema.types.length; tt++) {
              if (enumField === schema.types[tt].value) {
                if (schema.types[tt].properties[schema.field] === undefined)
                  // Don't throw an error when we encounter the original enum field
                  schema.types[tt].properties[schema.field] = {
                    type: "string",
                    output: "hidden"
                  };
                errors.concat(
                  checkObj(
                    value,
                    schema.types[tt].properties,
                    path + "." + schema.field + "=" + enumField,
            ...
            if (schema.items) {
              if (schema.items instanceof Array) {
                for (i = 0, l = value.length; i < l; i++) {
                  errors.concat(
                    checkProp(value[i], schema.items[i], path, i, recursion)
                  );
                }
              } else {
                ...
                for (i = 0, l = value.length; i < l; i++) {
                  ...
                  errors.concat(
                    checkProp(value[i], schema.items, path, i, nextRecursion)
                  );
                }
              }
            }
      ...
      for (var i in objTypeDef) {
        if (objTypeDef.prototype.hasOwnProperty.call(i)) {
          ...
          if (propDef.output !== "hidden" && value !== undefined) {
            if (value === null) addPrint(recursion, i + ": null");
            else if (typeof value === "object") addPrint(recursion, i + ":");
            else addPrint(recursion, i + ": " + value);
          }
    ...
    function checkObj(instance, objTypeDef, path, additionalProp, recursion) {
      ...
      for (var i in instance) {
        if (
          instance.prototype.hasOwnProperty.call(i) &&
          ...
          if (objTypeDef[i]) {
            ...
            if (!_changing && value && value.$schema) {
              errors = errors.concat(
                checkProp(value, value.$schema, path, i, recursion)
              );
            }
