      ...
      switch (element.el_type) {
        case "note":
          ...
          if (element.chord) {
            ...
            for (var c = 0; c < element.chord.length; c++) {
              if (element.chord[c].position === "default") {
                ...
                breakSynonyms.indexOf(element.chord[c].name.toLowerCase()) >= 0
              ) {
                ret += element.chord[c].name + " ";
              }
            }
          ...
          if (element.rest) {
            ...
            if (element.startTriplet) ret += "startTriplet\n\t\t";
            ...
            if (element.gracenotes) {
              ...
              for (var g = 0; g < element.gracenotes.length; g++) {
                ret +=
                  "(" +
                  element.gracenotes[g].pitch +
                  "," +
                  element.gracenotes[g].duration +
                  ") ";
              }
              ret += "\n\t\t";
            }
            ...
            for (var n = 0; n < element.pitches.length; n++) {
              ...
              if (element.pitches[n].startTie) ret += " startTie";
              if (element.pitches[n].endTie) ret += " endTie";
            }
            if (element.style && element.style !== "normal")
              ret += "\n\t\tstyle: " + element.style;
            if (element.noChordVoice) ret += "\n\t\tnoChordVoice";
          }
          ret += "\n";
          break;
        case "key":
          ...
          for (var k = 0; k < element.accidentals.length; k++) {
            ret +=
              "[" +
              element.accidentals[k].note +
              " " +
              element.accidentals[k].acc +
              "] ";
          }
          ret += "\n";
          break;
