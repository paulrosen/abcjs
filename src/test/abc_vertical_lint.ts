...
var verticalLint = function (tunes: any) {
  ...
  function fixed(digits: any, zero: any, num: any) {
    ...
    if (str.indexOf(zero) > 0) return str.split(".")[0];
  ...
  function getType(obj: any) {
    if (obj.$type.indexOf("staff-extra") >= 0) {
      if (obj.elem.length === 1) {
        ...
        switch (obj.elem[0]) {
          case "symbol clefs.G":
            if (obj.elem[1] === "symbol 8") return "Treble Clef 8";
            else if (obj.elem[1].indexOf("barNumber") === 0)
              return "Treble Clef [bar=" + obj.elem[1].substring(10) + "]";
            break;
          case "symbol clefs.F":
            if (obj.elem[1] === "symbol 8") return "Bass Clef 8";
            break;
          case "symbol clefs.C":
            if (obj.elem[1] === "symbol 8") return "C Clef 8";
            break;
        }
      ...
      if (
        obj.elem[0] === "symbol accidentals.sharp" ||
        obj.elem[0] === "symbol accidentals.flat" ||
        obj.elem[0] === "symbol accidentals.nat" ||
        obj.elem[0] === "symbol accidentals.halfsharp" ||
        obj.elem[0] === "symbol accidentals.halfflat"
      )
  ...
  function formatVoices(arr: any, indent: any) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      if (obj.beams.length > 0) str += formatBeams(obj.beams, indent + 1);
    }
    return tabs + str;
  }
  ...
  function formatOtherChildren(arr: any, indent: any) {
    ...
    for (i = 0; i < arr.length; i++) {
      ...
      str +=
        arr[i].type + " (" + params + ")" + formatClasses(arr[i].params) + "\n";
    }
    return str + "\n";
  }
  ...
  function formatObject(obj: any, indent: any) {
    ...
    if (typeof obj === "string" || typeof obj === "number") return tabs + obj;
    ...
    for (var key in obj) {
      if (obj.prototype.hasOwnProperty.call(key)) {
        ...
        else if (typeof value === "number") str.push(prefix + fixed1(value));
        else str.push(prefix + value);
      }
    }
    str = str.sort();
    return str.join("\n");
  ...
  function formatLine(line: any, lineNum: any) {
    ...
    if (line.brace) {
      for (var i = 0; i < line.brace.length; i++)
        str +=
          "brace: " +
          fixed1(line.brace[i].x) +
          " " +
          formatY(line.brace[i]) +
          addHeader(line.brace[i]) +
          "\n";
    }
    if (line.bracket) {
      for (var i2 = 0; i2 < line.bracket.length; i2++)
        str +=
          "bracket: " +
          fixed1(line.bracket[i2].x) +
          " " +
          formatY(line.bracket[i2]) +
          addHeader(line.bracket[i2]) +
          "\n";
    }
  ...
  function extractPositioningInfo(staffGroup: any, lineNum: any) {
    ...
    for (i = 0; i < staffGroup.voices.length; i++) {
      ...
      var obj = {
        bottom: fixed1(voice.bottom),
        top: fixed1(voice.top),
        specialY: setSpecialY(voice),
        width: fixed1(voice.w),
        startX: voice.startX,
        voiceChildren: [],
        otherChildren: [],
        beams: []
      ...
      for (var j = 0; j < voice.children.length; j++) {
        ...
        if (child.elemset) {
          for (var jj = 0; jj < child.elemset.length; jj++) {
            ...
            if (cl && cl.length > 0) {
              for (var jjj = 0; jjj < cl.length; jjj++) classes.push(cl[jjj]);
            }
          }
        }
        if (type === "note" || type === "rest") {
          if (child.abcelem.pitches) {
            var pitches = [];
            for (var ii = 0; ii < child.abcelem.pitches.length; ii++)
              pitches.push(child.abcelem.pitches[ii].verticalPos);
            type += "(" + pitches.join(",") + ")";
          } else {
            var r = "rest";
          ...
          if (child.abcelem.lyric && child.abcelem.lyric.length > 0)
            type += " " + child.abcelem.lyric[0].syllable;
        }
        var obj2 = {
          $type: type,
          bottom: fixed1(child.bottom),
          top: fixed1(child.top),
          specialY: setSpecialY(child),
        ...
        if (type === "tempo") {
          ...
          var arr = [];
          if (tempo.preString) arr.push(tempo.preString);
          else arr.push("*");
          arr.push(tempo.duration);
          if (tempo.postString) arr.push(tempo.postString);
          else arr.push("*");
          if (tempoNote) {
            arr.push(
              " note(" +
                fixed1(tempoNote.top) +
                "," +
                fixed1(tempoNote.bottom) +
                "," +
                fixed1(tempoNote.x) +
                "," +
                fixed1(tempoNote.w) +
  ...
  for (var i = 0; i < tunes.length; i++) {
    ...
    for (var j = 0; j < tune.lines.length; j++) {
      ...
      else positioning.push(JSON.stringify(line));
    }
  }
