...
var Tune = function () {
  ...
  this.getPickupLength = function () {
    ...
    return pickupLength < 1e-8 || barLength - pickupLength < 1e-8
      ? 0
      : pickupLength;
  };
      ...
      if (isTiedState !== undefined) {
        ...
        if (!eventHash["event" + voiceTimeMilliseconds]) {
          ...
          if (eventHash["event" + voiceTimeMilliseconds].left)
            eventHash["event" + voiceTimeMilliseconds].left = Math.min(
              eventHash["event" + voiceTimeMilliseconds].left,
              element.x
            );
          else eventHash["event" + voiceTimeMilliseconds].left = element.x;
          eventHash["event" + voiceTimeMilliseconds].elements.push(es);
          eventHash["event" + voiceTimeMilliseconds].startCharArray.push(
            element.abcelem.startChar
          );
          eventHash["event" + voiceTimeMilliseconds].endCharArray.push(
            element.abcelem.endChar
          );
          if (eventHash["event" + voiceTimeMilliseconds].startChar === null)
            eventHash["event" + voiceTimeMilliseconds].startChar =
              element.abcelem.startChar;
          if (eventHash["event" + voiceTimeMilliseconds].endChar === null)
            eventHash["event" + voiceTimeMilliseconds].endChar =
              element.abcelem.endChar;
          if (
            element.abcelem.midiPitches &&
            element.abcelem.midiPitches.length
          ) {
            ...
            for (var i = 0; i < element.abcelem.midiPitches.length; i++)
              eventHash["event" + voiceTimeMilliseconds].midiPitches.push(
                element.abcelem.midiPitches[i]
              );
          }
          if (
            element.abcelem.midiGraceNotePitches &&
            element.abcelem.midiGraceNotePitches.length
          ) {
            if (
              !eventHash["event" + voiceTimeMilliseconds].midiGraceNotePitches
            )
            ...
            for (
              var j = 0;
              j < element.abcelem.midiGraceNotePitches.length;
              j++
            )
              eventHash[
                "event" + voiceTimeMilliseconds
              ].midiGraceNotePitches.push(
                element.abcelem.midiGraceNotePitches[j]
              );
          }
        }
        if (nextIsBar) {
        ...
        for (var v = 0; v < voices.length; v++) {
          ...
          for (var elem = 0; elem < elements.length; elem++) {
            ...
            if (elements[elem].type === "bar" && noteFound)
              ...
              measureNumber[v]++;
            if (
              elements[elem].type === "note" ||
              elements[elem].type === "rest"
            )
              noteFound = true;
          }
        }
    ...
    for (var v = 0; v < voices.length; v++) {
      ...
      for (var elem = 0; elem < elements.length; elem++) {
        ...
        var ret = this.addElementToEvents(
          ...
          voiceTimeMilliseconds,
          elements[elem].top,
          elements[elem].height,
          elements[elem].line,
          elements[elem].measureNumber,
          timeDivider,
          isTiedState,
          nextIsBar
        );
        ...
        if (element.type === "bar") {
          ...
          var endRepeat =
            barType === "bar_right_repeat" || barType === "bar_dbl_repeat";
          var startEnding = element.abcelem.startEnding === "1";
          var startRepeat =
            barType === "bar_left_repeat" ||
          ...
          if (endRepeat) {
            ...
            for (var el2 = startingRepeatElem; el2 < endingRepeatElem; el2++) {
              ...
              if (
                tempoDone !== thisMeasure &&
                this.tempoLocations[thisMeasure]
              ) {
                bpm = this.tempoLocations[thisMeasure];
                timeDivider = (warp * this.getBeatLength() * bpm) / 60;
                tempoDone = thisMeasure;
      ...
      maxVoiceTimeMilliseconds = Math.max(
        maxVoiceTimeMilliseconds,
        voiceTimeMilliseconds
      );
    }
  ...
  function addEndPoints(lines, elements) {
    ...
    for (var i = 0; i < elements.length - 1; i++) {
      ...
      if (el.left !== null) {
        ...
        var endX =
          next && el.top === next.top ? next.left : lines[el.line].staffGroup.w;
  ...
  this.getBpm = function (tempo) {
    ...
    if (tempo) {
      ...
      var statedBeatLength =
        tempo.duration && tempo.duration.length > 0
          ? tempo.duration[0]
          : beatLength;
  ...
  this.setTiming = function (bpm, measuresOfDelay) {
    ...
    var startingDelay =
      ((measureLength / beatLength) * measuresOfDelay) / beatsPerSecond;
    if (startingDelay)
    ...
    if (this.noteTimings.length > 0) {
      this.totalTime =
        this.noteTimings[this.noteTimings.length - 1].milliseconds / 1000;
