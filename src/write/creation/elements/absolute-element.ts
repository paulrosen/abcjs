...
export default class AbsoluteElement implements types.AbsoluteElement {
	tuneNumber: number;
	...
	type: types.AbsoluteElement["type"];
	fixed: { w: number; t: number | undefined; b: number | undefined };
	specialY: types.AbsoluteElement["specialY"];
	hint: boolean;
	...
	constructor(
		...
		type: types.AbsoluteElement["type"],
		tuneNumber: number,
		options?: { durationClassOveride?: number },
	) {
		...
		this.durationClass = options.durationClassOveride
	...
	addExtra(extra: RelativeElement) {
	...
	addHead(head: RelativeElement) {
		if (head.dx < this.extraw) this.extraw = head.dx;
		this.heads[this.heads.length] = head;
		this.addRight(head);
	}

	addRight(right: RelativeElement) {
	...
	addFixedX(elem: RelativeElement) {
	...
	addCentered(elem: RelativeElement) {
	...
	setLimit(member: string, child: RelativeElement) {
	...
	private _addChild(child: RelativeElement) {
	...
	pushBottom(bottom: number | undefined) {
		if (bottom !== undefined) {
	...
	setX(x: number) {
		this.x = x;
		for (let i = 0; i < this.children.length; i++) this.children[i].setX(x);
	}

	// Not entirely sure if this should be "AbsoluteElement" or "RelativeElement"
	center(before: AbsoluteElement, after: AbsoluteElement) {
	...
	highlight(klass?: string, color?: string) {
		highlight.bind(this)(klass, color);
	}

	unhighlight(klass?: string, color?: string) {
