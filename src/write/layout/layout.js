...
var setXSpacing = function (
  ...
  for (var it = 0; it < 8; it++) {
    ...
    newspace = calcHorizontalSpacing(
      isLastLine,
      formatting.stretchlast,
      width + renderer.padding.left,
      staffGroup.w,
      newspace,
      ret.spacingUnits,
      ret.minSpace,
      renderer.padding.left + renderer.padding.right
    );
    if (debug)
...
function centerWholeRests(voices) {
  ...
  for (var i = 0; i < voices.length; i++) {
    ...
    for (var j = 1; j < voice.children.length - 1; j++) {
      ...
      if (
        absElem.abcelem.rest &&
        (absElem.abcelem.rest.type === "whole" ||
          absElem.abcelem.rest.type === "multimeasure")
      ) {
