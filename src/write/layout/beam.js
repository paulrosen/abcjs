...
var layoutBeam = function (beam) {
  ...
  var referencePitch = beam.stemsUp
    ? firstElement.abcelem.maxpitch
    : firstElement.abcelem.minpitch;
  ...
  var yPos = calcYPos(
    ...
    minStemHeight,
    beam.stemsUp,
    firstElement.abcelem.averagepitch,
    lastElement.abcelem.averagepitch,
    beam.isflat,
    beam.min,
    beam.max,
    beam.isgrace
  ...
  var beams = createAdditionalBeams(
    beam.elems,
    beam.stemsUp,
    beam.beams[0],
    beam.isgrace,
...
function calcYPos(
  ...
  var pos = Math.round(
    asc
      ? Math.max(average + barpos, maxPitch + barminpos)
      : Math.min(average - barpos, minPitch - barminpos)
  );
  ...
  for (var i = 0; i < elems.length; i++) {
    ...
    for (var durlog = getDurlog(duration); durlog < -3; durlog++) {
      ...
      if (i > 0 && elem.abcelem.beambr && elem.abcelem.beambr <= index + 1) {
        ...
        if (
          auxBeams[index].split[auxBeams[index].split.length - 1] >= xPos[0]
        ) {
    ...
    for (var j = auxBeams.length - 1; j >= 0; j--) {
      if (
        i === elems.length - 1 ||
        getDurlog(elems[i + 1].abcelem.duration) > -j - 4
      ) {
        var auxBeamEndX = x;
