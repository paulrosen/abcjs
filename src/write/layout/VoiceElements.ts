...
VoiceElement.layoutOneItem = function (
  ...
  if (
    ...
    var overlaps =
      firstChild &&
      ((child.abcelem.maxpitch <= firstChild.abcelem.maxpitch + 1 &&
        child.abcelem.maxpitch >= firstChild.abcelem.minpitch - 1) ||
        (child.abcelem.minpitch <= firstChild.abcelem.maxpitch + 1 &&
          child.abcelem.minpitch >= firstChild.abcelem.minpitch - 1));
    // See if they can share a note head
    if (
      overlaps &&
      child.abcelem.minpitch === firstChild.abcelem.minpitch &&
      child.abcelem.maxpitch === firstChild.abcelem.maxpitch &&
      firstChild.heads &&
      firstChild.heads.length > 0 &&
      child.heads &&
    ...
    if (overlaps) {
      ...
      var firstChildNoteWidth =
        firstChild.heads && firstChild.heads.length > 0
          ? firstChild.heads[0].realWidth
          : firstChild.fixed.w;
      ...
      for (var j = 0; j < child.children.length; j++) {
        ...
        if (relativeChild.name.indexOf("accidental") < 0) {
          if (!relativeChild.adjustedWidth)
            relativeChild.adjustedWidth =
              relativeChild.dx + firstChildNoteWidth;
  ...
  if (er < extraWidth) {
    ...
    if (
      voice.i === 0 ||
      child.type !== "bar" ||
      (voice.children[voice.i - 1].type !== "part" &&
        voice.children[voice.i - 1].type !== "tempo")
    )
      x += extraWidth - er;
  }
  child.setX(x);
...
VoiceElement.updateIndices = function (voice) {
  if (!this.layoutEnded(voice)) {
    ...
    if (voice.children[voice.i].type === "bar")
      voice.durationindex = Math.round(voice.durationindex * 64) / 64; // everytime we meet a barline, do rounding to nearest 64th
    voice.i++;
  }
};
