export default class Classes {
	...
	public voiceNumber: number | null;
	public measureNumber: number | null;
	public measureTotalPerLine: number[];
	public noteNumber: number | null;
	...
	generate(c: string): string {
		...
		if (ret.length > 0) {
			ret = ret.join(" ").split(" "); // Some strings are compound classes - that is, specify more than one class in a string.
