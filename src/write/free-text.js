function FreeText(text, vskip, getFontAndAttr, paddingLeft, width, getTextSize) {
	...
	if (text === "") {	// we do want to print out blank lines if they have been specified.
		...
		this.rows.push({left: paddingLeft, text: text, font: 'textfont', klass: 'defined-text', anchor: "start"});
		...
		for (var i = 0; i < text.length; i++) {
			...
			this.rows.push({left: x, text: text[i].text, font: currentFont, klass: 'defined-text', anchor: "start"});
