...
var createNoteHead = function (abselem, c, pitchelem, options) {
  ...
  var accidentalSlot =
    options.accidentalSlot !== undefined ? options.accidentalSlot : [];
  var shouldExtendStem =
    options.shouldExtendStem !== undefined ? options.shouldExtendStem : false;
  var printAccidentals =
    options.printAccidentals !== undefined ? options.printAccidentals : true;
  ...
  if (c === undefined)
    ...
    if (pitchelem.printer_shift) {
      ...
      shiftheadx =
        dir === "down"
          ? -glyphs.getSymbolWidth(c) * scale + adjust
          : glyphs.getSymbolWidth(c) * scale - adjust;
    }
    var opts = {
      scalex: scale,
    ...
    if (flag) {
      ...
      abselem.addRight(
        new RelativeElement(
          flag,
          xdelta,
          glyphs.getSymbolWidth(flag) * scale,
          pos,
    ...
    for (; dot > 0; dot--) {
      ...
      abselem.addRight(
        new RelativeElement(
          ...
          glyphs.getSymbolWidth("dots.dot"),
          pitch + dotadjusty
        )
      );
    }
