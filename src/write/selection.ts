...
function findElementByCoord(self, x, y) {
  ...
  for (var i = 0; i < self.selectables.length && minDistance > 0; i++) {
    ...
    if (
      ...
      var horiz = Math.min(
        Math.abs(el.dim.left - x),
        Math.abs(el.dim.right - x)
      );
      ...
      var vert = Math.min(
        Math.abs(el.dim.top - y),
        Math.abs(el.dim.bottom - y)
      );
      if (vert < minDistance) {
        minDistance = vert;
        closestIndex = i;
      ...
      var dx =
        Math.abs(x - el.dim.left) > Math.abs(x - el.dim.right)
          ? Math.abs(x - el.dim.right)
          : Math.abs(x - el.dim.left);
      var dy =
        Math.abs(y - el.dim.top) > Math.abs(y - el.dim.bottom)
          ? Math.abs(y - el.dim.bottom)
          : Math.abs(y - el.dim.top);
...
function getBestMatchCoordinates(dim, ev, scale) {
  ...
  if (
    dim.x <= ev.offsetX &&
    dim.x + dim.width >= ev.offsetX &&
    dim.y <= ev.offsetY &&
    dim.y + dim.height >= ev.offsetY
  )
  ...
  if (epsilon < 3) return [ev.offsetX, ev.offsetY];
...
function getTarget(target) {
  ...
  while (!found) {
    if (!target.parentElement) found = true;
    else {
...
function getMousePosition(self, ev) {
  ...
  if (clickedOn >= 0) {
    ...
    box = getBestMatchCoordinates(
      self.selectables[clickedOn].svgEl.getBBox(),
      ev,
      self.scale
    );
    x = box[0];
...
function mouseDown(ev) {
  ...
  if (
    ...
    this.selectables[positioning.clickedOn]
  ) {
...
function notifySelect(target, dragStep, dragMax, dragIndex, ev) {
  ...
  for (var i = 0; i < this.listeners.length; i++) {
    this.listeners[i](
      target.absEl.abcelem,
      target.absEl.tuneNumber,
      classes.join(" "),
      analysis,
      {
        step: dragStep,
