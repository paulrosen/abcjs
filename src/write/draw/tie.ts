...
function drawTie(renderer, params, linestartx, lineendx, selectables) {
  ...
  if (params.anchor1) {
    klass +=
      "abcjs-start-m" +
      params.anchor1.parent.counters.measure +
      "-n" +
      params.anchor1.parent.counters.note;
  } else klass += "abcjs-start-edge";
  if (params.anchor2) {
    klass +=
      " abcjs-end-m" +
      params.anchor2.parent.counters.measure +
      "-n" +
      params.anchor2.parent.counters.note;
...
var layout = function (params, lineStartX, lineEndX) {
  ...
  if (!params.anchor1 || !params.anchor2)
    ...
    params.anchor1.pitch === params.anchor2.pitch &&
    params.internalNotes.length === 0
  )
    params.isTie = true;
