...
function drawTempo(renderer, params) {
	...
	renderer.wrapInAbsElem(params.tempo, "abcjs-tempo", function () {
		...
		if (params.tempo.preString) {
			text = renderText(renderer, {x:x, y: y, text: params.tempo.preString, type: 'tempofont', klass: 'abcjs-tempo', anchor: "start", noClass: true, history: "ignore"});
		...
		if (params.note) {
			...
			text = renderText(renderer, {x:x, y: y, text: str, type: 'tempofont', klass: 'abcjs-tempo', anchor: "start", noClass: true, history: "ignore"});
