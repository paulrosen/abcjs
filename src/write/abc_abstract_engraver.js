...
import createNoteHead from './abc_create_note_head';
...
AbstractEngraver.prototype.createABCStaff = function (
  ...
  for (var v = 0; v < abcstaff.voices.length; v++) {
    ...
    if (v === 0) {
      voice.barfrom =
        abcstaff.connectBarLines === "start" ||
        abcstaff.connectBarLines === "continue";
      voice.barto =
        abcstaff.connectBarLines === "continue" ||
        abcstaff.connectBarLines === "end";
    } else {
    ...
    if (abcstaff.title && abcstaff.title[v]) {
      ...
      voice.headerPosition =
        6 +
        staffgroup.getTextSize.baselineToCenter(
          voice.header,
          "voicefont",
    ...
    var clef =
      (!this.initialClef || l === 0) &&
      createClef(abcstaff.clef, this.tuneNumber);
    if (clef) {
    ...
    if (abcstaff.meter) {
      if (abcstaff.meter.type === "specified") {
        this.measureLength =
          abcstaff.meter.value[0].num / abcstaff.meter.value[0].den;
    ...
    var staffLines =
      abcstaff.clef.stafflines || abcstaff.clef.stafflines === 0
        ? abcstaff.clef.stafflines
        : 5;
...
AbstractEngraver.prototype.addGraceNotes = function (
  ...
  for (i = 0; i < elem.gracenotes.length; i++) {
    ...
    ledgerLines(
      ...
      gracepitch,
      false,
      glyphs.getSymbolWidth("noteheads.quarter"),
      [],
      true,
      grace.dx - 1,
      0.6
    ...
    var isInvisibleRest =
      elem.rest &&
      (elem.rest.type === "spacer" || elem.rest.type === "invisible");
...
function addRestToAbsElement(
  ...
  if (
    elem.rest.type.indexOf("multimeasure") < 0 &&
    elem.rest.type !== "invisible"
  ) {
...
AbstractEngraver.prototype.addNoteToAbcElement = function (
  ...
  var p;
  for (
    p = dir === "down" ? elem.pitches.length - 2 : 1;
    dir === "down" ? p >= 0 : p < elem.pitches.length;
    p = dir === "down" ? p - 1 : p + 1
  ) {
  ...
  for (p = 0; p < elem.pitches.length; p++) {
    ...
    var isBottomWhenStemIsUp =
      (stemdir === "down" || dir === "down") && p === pp - 1;
  ...
  if (hasStem) {
    ...
    var p1 =
      dir === "down" ? elem.minpitch - stemHeight : elem.minpitch + 1 / 3;
    // PER added stemdir test to make the line meet the note.
    if (p1 > 6 && !stemdir) p1 = 6;
    var p2 =
      dir === "down" ? elem.maxpitch - 1 / 3 : elem.maxpitch + stemHeight;
    // PER added stemdir test to make the line meet the note.
    if (p2 < 6 && !stemdir) p2 = 6;
    var dx =
      dir === "down" || abselem.heads.length === 0 ? 0 : abselem.heads[0].w;
...
AbstractEngraver.prototype.createNote = function (
  ...
  if (
    ...
    !elem.startTriplet &&
    !elem.endTriplet &&
    !(elem.rest && elem.rest.type === "spacer")
  ) {
...
AbstractEngraver.prototype.addSlursAndTies = function (
  ...
  if (pitchelem.endTie) {
    if (this.ties.length > 0) {
      ...
      for (var j = 0; j < this.ties.length; j++) {
        if (
          this.ties[j].anchor1 &&
          this.ties[j].anchor1.pitch === notehead.pitch
        ) {
  ...
  if (pitchelem.endSlur) {
    ...
    for (var s in this.slurs) {
      if (this.slurs.prototype.hasOwnProperty.call(s)) {
...
AbstractEngraver.prototype.createBarLine = function (
  ...
  var firstdots =
    elem.type === "bar_right_repeat" || elem.type === "bar_dbl_repeat";
  var firstthin =
    elem.type !== "bar_left_repeat" &&
    elem.type !== "bar_thick_thin" &&
    elem.type !== "bar_invisible";
  var thick =
    elem.type === "bar_right_repeat" ||
    elem.type === "bar_dbl_repeat" ||
    elem.type === "bar_left_repeat" ||
    elem.type === "bar_thin_thick" ||
    elem.type === "bar_thick_thin";
  var secondthin =
    elem.type === "bar_left_repeat" ||
    elem.type === "bar_thick_thin" ||
    ...
    elem.type === "bar_dbl_repeat";
  var seconddots =
    elem.type === "bar_left_repeat" || elem.type === "bar_dbl_repeat";
  ...
  if (elem.startEnding && isFirstStaff) {
    ...
    var textWidth = this.getTextSize.calc(
      elem.startEnding,
      "repeatfont",
      ""
    ).width;
