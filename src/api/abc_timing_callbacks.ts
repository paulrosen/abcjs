var TimingCallbacks = function (target, params) {
  ...
  self.extraMeasuresAtBeginning = params.extraMeasuresAtBeginning
    ? parseInt(params.extraMeasuresAtBeginning, 10)
    : 0;
  ...
  self.lineEndAnticipation = params.lineEndAnticipation
    ? parseInt(params.lineEndAnticipation, 10)
    : 0; // How many milliseconds before the end should the call happen.
  self.beatSubdivisions = params.beatSubdivisions
    ? parseInt(params.beatSubdivisions, 10)
  ...
  self.replaceTarget = function (newTarget) {
    ...
    if (self.lineEndCallback) {
      self.lineEndTimings = getLineEndTimings(
    ...
    self.lastMoment =
      self.noteTimings[self.noteTimings.length - 1].milliseconds;
  ...
  self.doTiming = function (timestamp) {
    ...
    if (!self.isPaused && self.isRunning) {
      ...
      while (
        ...
        self.noteTimings[self.currentEvent].milliseconds < self.currentTime
      ) {
        if (
          self.eventCallback &&
          self.noteTimings[self.currentEvent].type === "event"
        ) {
      ...
      if (
        ...
        self.lineEndTimings[self.currentLine].milliseconds < self.currentTime &&
  ...
  self.doBeatCallback = function (timestamp) {
    if (self.beatCallback) {
      ...
      while (
        next < self.noteTimings.length &&
        self.noteTimings[next].left === null
      )
        next++;
      var endMs;
      var ev;
  ...
  self.setProgress = function (position, units) {
    ...
    switch (units) {
      case "seconds":
        ...
        if (self.currentTime > self.lastMoment)
          self.currentTime = self.lastMoment;
        percent = self.currentTime / self.lastMoment;
        break;
      case "beats":
        self.currentTime =
          position * self.millisecondsPerBeat * self.beatSubdivisions;
        if (self.currentTime < 0) self.currentTime = 0;
        if (self.currentTime > self.lastMoment)
          self.currentTime = self.lastMoment;
        percent = self.currentTime / self.lastMoment;
    ...
    if (self.lineEndCallback)
      self.lineEndCallback(
        self.lineEndTimings[self.currentLine],
        self.noteTimings[self.currentEvent],
        { line: self.currentLine, endTimings: self.lineEndTimings }
      );
